  ...
  switch (category) {
    ...
    case "spreadsheet":
      if (input === "xlsx") {
        ...
        if (output === "json")
          return await xlsxToJSON(file, options, onProgress);
      }
      if (input === "csv") {
        if (output === "xlsx")
          return await csvToXLSX(file, options, onProgress);
        if (output === "json")
          return await csvToJSON(file, options, onProgress);
      }
      // Fallback
      throw new Error(
        `Conversion from ${input} to ${output} not supported yet for spreadsheets`,
      );

    case "data":
      if (input === "json") {
        if (output === "xlsx")
          return await jsonToXLSX(file, options, onProgress);
        if (output === "csv") return await jsonToCSV(file, options, onProgress);
        if (output === "yaml" || output === "yml")
          return await jsonToYAML(file, options, onProgress);
        if (output === "xml") return await jsonToXML(file, options, onProgress);
        if (output === "toml")
          return await jsonToTOML(file, options, onProgress);
      }
      if (input === "yaml" || input === "yml") {
        if (output === "json")
          return await yamlToJSON(file, options, onProgress);
      }
      if (input === "xml") {
        if (output === "json")
          return await xmlToJSON(file, options, onProgress);
      }
      if (input === "toml") {
        if (output === "json")
          return await tomlToJSON(file, options, onProgress);
      }
      throw new Error(
        `Conversion from ${input} to ${output} not supported yet for data files`,
      );
    ...
    case "document":
      ...
      throw new Error(
        `Conversion from ${input} to ${output} not supported yet for documents`,
      );
