...
interface ConversionState {
    ...
    addJob: (file: File, inputFormat: string, outputFormat: string, options: ConversionOptions) => string;
...
export const useConversionStore = create<ConversionState>((set, get) => ({
    ...
    addJob: (file, inputFormat, outputFormat, options) => {
        const id = generateId();
        const estimatedTime = estimateConversionTime(file.size, inputFormat, outputFormat);
...
if (typeof window !== 'undefined') {
    setInterval(() => {
        const { jobs, autoClearEnabled, autoClearDelay, removeJob } = useConversionStore.getState();
